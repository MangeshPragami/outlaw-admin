...
console.log('API_BASE:', process.env.REACT_APP_API_BASE); // Add this line
const API_BASE = process.env.REACT_APP_API_BASE || 'http://localhost:3001/'; // Updated port to match server
...
// In services/api.js
export const adminJoinMeeting = async (token, bookingId) => {
  const response = await fetch(`${API_BASE}/admin/create-meeting`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`
...
// In services/api.js  
export const getBookingTranscript = async (token, bookingId) => {
  const response = await fetch(`${API_BASE}/api/bookings/${bookingId}/transcript`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const getBookingsOverview = async (token, period = 'all') => {
  const response = await fetch(`${API_BASE}/api/bookings/overview?period=${period}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const getBookingsByStatus = async (token, status) => {
  const response = await fetch(`${API_BASE}/api/bookings/status/${status}`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
export const getUpcomingBookings = async (token) => {
  const response = await fetch(`${API_BASE}/api/bookings/upcoming`, {
    headers: { Authorization: `Bearer ${token}` }
  });
