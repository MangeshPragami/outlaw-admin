...
import { getAllBookingsAdmin, getBookingByIdAdmin } from './bookings/read.js';
import { updateBookingAPI } from './bookings/update.js';
...
export const app = async (event, context, requestId) => {
    ...
    switch (event.rawPath) {
        ...
        case ADMIN_API_PATHS.GET_ALL_BOOKINGS: {
            return await getAllBookingsAdmin(body);
        }
        case ADMIN_API_PATHS.GET_BOOKING_BY_ID: {
            return await getBookingByIdAdmin(body);
        }
        case ADMIN_API_PATHS.UPDATE_BOOKING: {
            return await updateBookingAPI(body);
        }
        case ADMIN_API_PATHS.DELETE_BOOKING: {
            // Delete by setting status to cancelled
