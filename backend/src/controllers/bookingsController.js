...
export const getBookingTranscript = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Return transcript URL or fetch from S3
    res.json({ 
      url: booking.transcript_url,
      chime_data: booking.chime_meeting_response 
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
