...
const FILE_NAME = 'create-meeting.js';
...
export async function createMeeting(body) {
    ...
    try {
        ...
        // @todo check if a current user body.userId is either creatorId or participantId in bookings table, if doesn't exists throw an error.

        // if booking.start_time is <= current time, then go ahead, else throw an error.
        
        const booking = bookingResult.data.booking;

        // Check if current user is either creator or participant
        if (body.userId !== booking.creator_id && body.userId !== booking.participant_id) {
            return {
                statusCode: 403,
                body: { message: 'You do not have permission to join this meeting.' }
            };
        }
        ...
        // Check if it's the right time to join the meeting
        const currentTime = new Date();
        const meetingStartTime = new Date(booking.start_time);
        const meetingEndTime = new Date(booking.end_time);
        const timeBuffer = 5 * 60 * 1000; // 5 minutes buffer before start time

        // Check if meeting time has passed
        if (currentTime.getTime() > meetingEndTime.getTime()) {
            return {
                statusCode: 400,
                body: { 
                    message: `Meeting has ended. This meeting was scheduled from ${meetingStartTime.toLocaleTimeString()} to ${meetingEndTime.toLocaleTimeString()}.`
                }
            };
        }

        // Check if it's too early for the meeting
        if (currentTime.getTime() < meetingStartTime.getTime() - timeBuffer) {
            const minutesUntilStart = Math.ceil((meetingStartTime - currentTime) / (60 * 1000));
            return {
                statusCode: 400,
                body: { 
                    message: `Meeting cannot be created yet. Please wait until 5 minutes before the scheduled start time. Meeting starts in ${minutesUntilStart} minutes.`
                }
            };
        }
        ...
        if (booking.chime_meeting_response) {
            const selfAttendeeData = booking.chime_meeting_response.attendeeResponses.find((item) => {
                if(item.Name === currentUser.email) {
                    item.Attendee.Name = currentUser.email
                    return item
                }
            });
            return {
                ...
                body: {
                    ...
                    Attendee: selfAttendeeData.Attendee,
                }
            };
        }
        ...
        // @todo need to add live transcription media pipeline code here similar to admin/bookings/create-meeting.js file.
        // @todo use env var to get account id
        const AWS_ACCOUNT_ID = "069717477936";
        ...
        try {
            const pipelineCommand = new CreateMediaCapturePipelineCommand({
                ...
                SinkType: 'S3Bucket',
                SinkArn: `arn:aws:s3:::outlaw-meeting-transcripts-dev`, // Replace with your bucket
        ...
        const selfAttendeeData = attendeeResponses.find((item) => {
                if(item.Name === currentUser.email) {
                    item.Attendee.Name = currentUser.email
                    return item
                }
